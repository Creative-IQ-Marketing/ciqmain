...
const PLATFORMS = ["ChatGPT", "Gemini", "Claude", "Perplexity", "Google AI", "Voice Search"];

const AUDIT = [
  { id: 1, label: "AI Discoverability", q: "Are you showing up where your customers are actually searching—on AI platforms, not just Google?" },
  { id: 2, label: "Lead Conversion", q: "Is your website built to convert visitors into qualified leads, or just exist online?" },
  { id: 3, label: "Chat Experience", q: "Does your chat feel like real, high-level customer service—or a missed opportunity?" },
  { id: 4, label: "Social Positioning", q: "Is your social media positioning your brand as the obvious choice, or just adding noise?" },
];
  ...
  return (
    ...
    >
        ...
        <svg className="absolute inset-0 h-full w-full opacity-[0.06]" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="grid" width="56" height="56" patternUnits="userSpaceOnUse">
              <path d="M 56 0 L 0 0 0 56" fill="none" stroke="rgb(15 23 42)" strokeWidth="0.5" />
            </pattern>
          </defs>
      ...
      <motion.div
        ...
        className="relative z-10 mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-center px-5 pb-16 pt-28 sm:px-8 sm:pb-20 sm:pt-32 lg:px-10"
      >
        {/* Badge */}
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
          <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white/70 px-3.5 py-1.5 text-[11px] font-semibold uppercase tracking-widest text-slate-500 shadow-sm backdrop-blur-sm">
        ...
        {/* Grid: stacked on mobile, two-col on lg */}
        <div className="mt-10 grid gap-10 sm:mt-12 lg:grid-cols-[1fr_400px] lg:gap-14 xl:grid-cols-[1fr_440px]">
          ...
          <div className="flex flex-col">
            <motion.h1
              ...
              className="text-[2.6rem] font-extrabold leading-[0.95] tracking-tight sm:text-[3.8rem] lg:text-[4.8rem] xl:text-[5.2rem]"
            >
              ...
              <span className="dm mt-2 block text-[1.15rem] font-light text-slate-600 sm:text-[1.4rem] lg:text-[1.6rem]">
            ...
            >
              One unified system across your website, socials, and CRM — built for{" "}
              <span className="text-slate-900/80">
                <TypewriterText texts={["AI-driven discovery.", "search dominance.", "real conversions.", "agents that convert."]} />
              </span>
            </motion.p>
            ...
            >
              {METRICS.map((m, i) => (
                <div key={i} className="flex flex-col">
                  <span className="text-2xl font-extrabold text-slate-950 sm:text-3xl">{m.value}</span>
                  <span className="dm mt-0.5 text-[10px] uppercase tracking-widest text-slate-500 sm:text-xs">{m.sub}</span>
                </div>
              ))}
            ...
            >
              <motion.a
                href="#contact"
                onClick={() => trackButtonClick("Consultation", "hero_cta_primary", "Hero")}
                className="group inline-flex items-center justify-center gap-2 rounded-xl bg-blue-500 px-6 py-3.5 text-sm font-semibold text-white shadow-lg shadow-blue-500/20 transition-all hover:bg-blue-400"
              ...
              <motion.a
                href="#services"
                onClick={() => trackButtonClick("Services", "hero_cta_secondary", "Hero")}
                className="inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white/70 px-6 py-3.5 text-sm font-semibold text-slate-700 shadow-sm backdrop-blur transition-all hover:border-slate-300 hover:bg-white hover:text-slate-900"
                whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
              >
                View Services
              </motion.a>
            ...
            {/* Platform strip */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.75 }} className="mt-10 overflow-hidden">
              <p className="dm mb-3 text-[10px] uppercase tracking-widest text-slate-500">Visible across</p>
          ...
          <motion.div
            ...
            transition={{ duration: 0.85, delay: 0.38, type: "spring", stiffness: 70 }}
            ...
            <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white/70 shadow-xl shadow-slate-900/5 backdrop-blur-xl">
              <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3.5 sm:px-5">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-semibold text-slate-900">Quick Audit</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-400 animate-pulse" />
                  <span className="dm text-[11px] text-slate-500">AI-Ready Check</span>
                </div>
              </div>
              ...
              <div className="divide-y divide-slate-200 p-1.5 sm:p-2">
                {AUDIT.map((item, i) => (
                  <motion.a
                    key={item.id}
                    href="#contact"
                    onClick={() => trackButtonClick("Audit Question", `hero_audit_${item.id}`, "Hero")}
                  ...
                  >
                    <span
                      className={`dm mt-0.5 shrink-0 text-xs font-medium tabular-nums transition-colors duration-200 ${
                        activeAudit === item.id ? "text-blue-700" : "text-slate-300"
                      }`}
                    >
                      0{item.id}
                    </span>
                    <div className="min-w-0 flex-1">
                      <div
                        className={`dm mb-1 text-[10px] font-semibold uppercase tracking-widest transition-colors duration-200 ${
                          activeAudit === item.id ? "text-blue-700" : "text-slate-500"
                        }`}
                      >
                        {item.label}
                      </div>
                      <p className="dm text-[13px] leading-relaxed text-slate-700 transition-colors duration-200 group-hover:text-slate-900 sm:text-sm">
                        {item.q}
                      </p>
                    </div>
                    <ArrowUpRight
              ...
              <div className="border-t border-slate-200 px-4 py-3.5 sm:px-5">
                <p className="dm text-xs leading-relaxed text-slate-600">
                  ...
                  <a href="#contact" className="text-blue-700/80 transition-colors hover:text-blue-700">
                    Let's fix that.
                  </a>
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-px bg-slate-200">
                <div className="bg-white/80 px-4 py-3">
                  <div className="dm mb-1.5 text-[10px] uppercase tracking-widest text-slate-500">AI Indexing</div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="dm text-xs font-medium text-slate-600">92%</span>
                  </div>
                </div>
                <div className="bg-white/80 px-4 py-3">
                  <div className="dm mb-1.5 text-[10px] uppercase tracking-widest text-slate-500">Lead Capture</div>
                  <div className="flex items-center gap-1.5">
